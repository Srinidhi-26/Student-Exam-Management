...
@studentmanagement_api.errorhandler(StudentException)
def handle_exception(e):
    app.logger.exception(e)
    return {"success": False, "error": e.message}, e.code


@studentmanagement_api.errorhandler(SQLAlchemyError)
...
def get_student(student_id=None):
    ...
    paginated_students = students.paginate(page=page_num, error_out=False)
...
@studentmanagement_api.route("/create", methods=["POST"])
def create_student():
    now = dt.now(timezone.utc)
    data = request.json
    name = data.get("name")
